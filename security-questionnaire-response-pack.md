...
**Sample response:**
FinSight AI enforces tenant isolation at the data, application, and AI layers. At the data layer, each customer's data is stored in a logically separate database schema with row-level access controls tied to a customer-specific tenant ID. At the application layer, API request handling enforces tenant ID validation on every request -- a session authenticated for Customer A cannot retrieve data associated with Customer B. At the AI layer, conversation context and prompt history are scoped to individual sessions and are not shared across tenants or used to inform other customers' queries. Tenant isolation controls are included in the scope of our annual penetration test, and findings related to logical separation are covered in our SOC 2 Type II report.
...
**Sample response:**
FinSight AI has conducted an internal risk classification review under the EU AI Act and has assessed its current product as a limited-risk AI system under Article 50. This classification is based on analysis of the use cases the platform supports. None of the currently supported workflows fall within the high-risk categories defined in Annex III -- such as AI used to make or materially influence credit scoring decisions, employment determinations, or critical infrastructure management -- because FinSight AI is designed to support analyst review rather than generate autonomous decisions in those domains.

At the limited-risk tier, our primary transparency obligation is disclosure: users are informed when they are interacting with AI-generated output. This disclosure is implemented at the product level and documented in our user-facing terms of service.

We maintain technical documentation and a system description consistent with EU AI Act transparency requirements and have engaged external legal counsel to validate our classification approach. We are prepared to share our classification rationale and supporting documentation with customers on request. Customers whose use of the platform involves high-impact decision workflows are encouraged to conduct their own classification review in consultation with legal counsel.
